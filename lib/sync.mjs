...
function resetWatermarkTurns(cwd) {
  ...
  try {
    ...
    if (e.message && e.message.includes('rename failed')) return; // already logged
...
export class GitSync {
  ...
  push(cwd, sessionId) {
    ...
    // Guard: if user has staged files outside .relay/, temporarily unstage them so relay
    // doesn't push user code under a [relay] commit message without their knowledge.
    let nonRelayStaged = [];
    try {
      const staged = git(['diff', '--cached', '--name-only'], cwd, { timeout: 2_000 });
      nonRelayStaged = (staged || '').split('\n').filter((f) => f && !f.startsWith('.relay/'));
