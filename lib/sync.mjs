...
export class GitSync {
  ...
  pull(cwd, { fetchTimeoutMs = 3_000, checkoutTimeoutMs = 2_000 } = {}) {
    // env escape hatch for local-only or demo environments
    if (process.env.WYREN_SKIP_PULL) return;

    // short-circuit if no remote configured — avoids 50ms spawn + guaranteed failure
    try {
  ...
  lock(cwd) {
    ...
    // 'wx' flag is atomic — fails with EEXIST if file already exists (no TOCTOU window)
    try {
