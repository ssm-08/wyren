...
// File edits are strong signal — team changed something deliberately (ground truth, not hypothetical)
...
function structuralScore(lines) {
  ...
  if (editCount >= 2) score += 2;
  if (editCount >= 5) score += 3;
  if (editCount >= 10) score += 3;
...
function getThreshold() {
  const p = parseInt(process.env.WYREN_TIER0_THRESHOLD ?? '3', 10);
  return isNaN(p) ? 3 : p;
}
...
export function hasTier0Signal(transcriptText, lines = []) {
  ...
  process.stderr.write(`[wyren] tier0 score=${score} threshold=${threshold} passes=${passes}\n`);
  return passes;
}
