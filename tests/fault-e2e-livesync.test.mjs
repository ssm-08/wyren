 ...
 *
 * End-to-end simulation tests for the Wyren live-sync feature.
...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'wyren-livesync-'));
}
 ...
 * Does NOT create any local branches yet — initRepoWyren does the first commit.
 */
...
/**
 * Create initial .wyren/memory.md in repoDir and push to origin.
 * Establishes the remote branch (master or main, whatever git init uses).
 * Returns the branch name that was pushed.
 */
function initRepoWyren(repoDir, initialContent = '# Wyren Memory\n') {
  const wyrenDir = path.join(repoDir, '.wyren');
  fs.mkdirSync(wyrenDir, { recursive: true });
  fs.writeFileSync(path.join(wyrenDir, 'memory.md'), initialContent, 'utf8');

  git(['add', '.wyren/memory.md'], repoDir);
  git(['commit', '-m', 'init wyren'], repoDir);

  const r = git(['push', '--set-upstream', 'origin', 'HEAD'], repoDir);
  if (r.status !== 0) {
    throw new Error(`initRepoWyren push failed: ${r.stderr}`);
  }
 ...
 */
function cloneWyren(repoDir, remoteBranch) {
  const branch = remoteBranch || detectBranch(repoDir);

  const fetchR = git(['fetch', 'origin'], repoDir);
  if (fetchR.status !== 0) {
    throw new Error(`cloneWyren fetch failed: ${fetchR.stderr}`);
  }
  ...
  if (coR.status !== 0) {
    throw new Error(`cloneWyren checkout failed: ${coR.stderr}`);
  }
...
function pushMemory(repoDir, content) {
  ...
  const commitR = git(['commit', '-m', '[wyren] test push'], repoDir);
  ...
  if (pushR.status !== 0) {
    ...
    const attrEntry = '.wyren/memory.md merge=ours\n';
    let existingAttrs = '';
    try { existingAttrs = fs.readFileSync(gitAttributesPath, 'utf8'); } catch {}
    if (!existingAttrs.includes('.wyren/memory.md')) {
...
function seedBUpsState(repoDir, seenContent) {
  const stateDir = path.join(repoDir, '.wyren', 'state');
  fs.mkdirSync(stateDir, { recursive: true });

  const memPath = path.join(repoDir, '.wyren', 'memory.md');
  // Use stale mtime (1) so mtime guard doesn't fast-path — actual mtime check
  // will see a difference because the file was written by cloneWyren (not by us).
 ...
 * opts.skipPull: set WYREN_SKIP_PULL=1 to skip git fetch.
...
function runUPS(repoDir, opts = {}) {
  ...
  if (opts.skipPull) env.WYREN_SKIP_PULL = '1';
  else delete env.WYREN_SKIP_PULL;
...
test('Test 1: A pushes new memory, B UPS fires and injects delta', () => {
  ...
  try {
    ...
    // Step 1: A initialises .wyren and pushes to bare
    const v0 = '# Wyren Memory\n\n## Decisions\n- Use SQLite [session a, turn 1]\n';
    const branch = initRepoWyren(repoA, v0);

    // Step 2: B clones (creates local tracking branch so GitSync.pull works)
    cloneWyren(repoB, branch);

    const bMemPath = path.join(repoB, '.wyren', 'memory.md');
    assert.ok(fs.existsSync(bMemPath), 'B should have memory.md after cloneWyren');
    assert.ok(
    ...
    assert.ok(ctx.includes('Wyren live update'), 'Delta header must be present');
  } finally {
...
test('Test 5: No remote initially, remote added mid-session, UPS eventually injects', () => {
  ...
  try {
    ...
    const v0 = '# Wyren Memory\n\n## Decisions\n- Use SQLite [session a, turn 1]\n';
    initRepoWyren(repoA, v0);

    // Give repoB an initial commit so git has a local HEAD (needed for tracking setup later)
    fs.mkdirSync(path.join(repoB, '.wyren', 'state'), { recursive: true });
    fs.writeFileSync(path.join(repoB, '.wyren', 'memory.md'), v0, 'utf8');
    git(['add', '.wyren/memory.md'], repoB);
    ...
    const upsStatePath = path.join(repoB, '.wyren', 'state', 'ups-state.json');
    ...
    // cloneWyren uses --force to handle the pre-existing local memory.md.
    git(['remote', 'add', 'origin', bare], repoB);
    const detectedBranch = detectBranch(repoB);
    cloneWyren(repoB, detectedBranch);
